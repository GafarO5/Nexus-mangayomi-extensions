...
class DefaultExtension extends MProvider {
    ...
    async getLatestUpdates(page) {
        // Note: the site's homepage "Latest Updates" feed is rendered by
        // JavaScript and isn't present in the raw HTML a source receives, so it
        // can't be scraped directly. We approximate it with search sorted by
        // "Latest Updates" — a stable, server-rendered list of real series.
        const filters = this.getFilterList();
        filters[0].state = 5;
        return await this.search("", page, filters)
    }
    ...
    async search(query, page, filters) {
        ...
        for (var manga of mangaElements) {
            ...
            // Title: prefer the <abbr title> the site uses for series names,
            // fall back to the link text, then the old positional path.
            var name = "";
            var abbr = manga.selectFirst("abbr");
            if (abbr) name = (abbr.attr("title") || abbr.text || "").trim();
            if (!name && details) name = (details.text || "").trim();
            if (!name) {
                var d = manga.selectFirst("article > div > div > div");
                if (d) name = (d.text || "").trim();
            }
